...
KHIST_EPISODES = [
    dict(slug="gojoseon_e01", title="1화. 곰님, 100일만 참으세요!", lv=1,
         ...
         notes=[("전승", "단군 이야기는 고려 시대에 일연이 쓴 《삼국유사》(1281년)와 이승휴의 《제왕운기》에 실려 전해집니다."),
                ("전승", "《삼국유사》에는 환웅이 '백 일 동안 햇빛을 보지 말라'고 했지만, 곰은 삼칠일(21일) 만에 여자의 몸이 되었다고 적혀 있습니다."),
                ("전승", "웅녀가 낳은 아들 단군왕검이 아사달에 도읍을 정하고 나라 이름을 조선이라 했다고 전합니다."),
    ...
    dict(slug="sp_jeondeungsa", title="특별편. 꺼지지 않는 등불 — 전등사", lv=2,
         ...
         notes=[("전승", "고구려 소수림왕 때 아도 화상이 세웠고 처음 이름은 진종사였다고 전합니다."),
    ...
    dict(slug="sp_chamseongdan", title="특별편. 하늘에 닿은 제단 — 참성단", lv=2,
         ...
         notes=[("전승", "단군이 하늘에 제사를 지내려고 쌓았다고 전해 옵니다."),
                ("기록", "고려 시대부터 나라에서 이곳에 제사를 올린 기록이 남아 있고, 조선 시대에도 여러 차례 고쳐 쌓았습니다."),
                ("기록", "지금도 전국체육대회의 성화를 이곳에서 채화합니다.")],
         quiz=[{"q": "참성단이 있는 산은?",
    ...
    dict(slug="sp_bugeunri", title="특별편. 거인의 돌 탁자 — 부근리 고인돌", lv=2,
         ...
         notes=[("기록", "위에 얹은 덮개돌은 길이가 6미터가 넘고 무게는 수십 톤으로 추정됩니다."),
                ("기록", "이런 모양을 탁자식(북방식) 고인돌이라고 부릅니다."),
                ("기록", "2000년에 고창·화순·강화의 고인돌 유적이 유네스코 세계유산으로 등재되었습니다."),
                ("상상", "돌을 어떻게 옮기고 올렸는지는 정확한 기록이 없어, 통나무와 흙 언덕을 쓰는 방법 등이 추정되고 있습니다.")],
    ...
    dict(slug="sp_osangri", title="특별편. 열두 기의 돌 가족 — 오상리 고인돌", lv=2,
         ...
         notes=[("기록", "발굴 조사에서 고인돌 아래 무덤방과 함께 간석기·토기 조각 등이 나왔습니다."),
                ("기록", "부근리 고인돌과 같은 탁자식이지만 크기는 훨씬 작습니다."),
    ...
    dict(slug="sp_ganghwa_church", title="특별편. 배를 닮은 한옥 교회 — 강화성당", lv=3,
         ...
         notes=[("기록", "대한성공회 강화성당은 1900년에 세워진, 현재 남아 있는 가장 오래된 한옥 성당입니다."),
                ("기록", "밖은 한옥의 팔작지붕, 안은 서양 교회의 바실리카 구조로 되어 있습니다."),
                ("전승", "언덕 위 자리와 긴 건물 모양이 배를 닮았다 하여 '노아의 방주'에 빗대어 이야기합니다.")],
         quiz=[{"q": "강화성당이 지어진 해는?",
...
MOZART_UNITS = [
    ...
    ("🎻 협주곡", ["K175", "K175_long", "K219", "K466", "K467", "K488"], 3),
]
...
def build_mozart():
    ...
    units, n_vid, skipped = [], 0, []
    for key, v in up.items():
        if key.startswith("_") or v.get("privacy") != "unlisted":
            skipped.append("%s(%s)" % (key, v.get("privacy", "?")))
    ...
    for unit_nm, keys, lv in MOZART_UNITS:
        ...
        for k in keys:
            ...
            html.append('<p style="margin-top:10px;font-size:11px;color:var(--t3)">'
                        '✅ 이 영상은 실제 악보에서 만든 MIDI인지 4중 검증(음정·조성·곡·템포)을 통과한 것만 실었습니다.</p>')
