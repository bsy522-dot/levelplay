...
MATH_EPISODES = [
    ...
    dict(slug="s2e02", no=7, title="끝없는 선을 기호로", theme="이집트 — 숫자의 발명", lv=2,
         novel="07_끝없는선을기호로_기수법.txt",
         pages=["S2E02_끝없는선을기호로_9컷_%d쪽.png" % i for i in range(1, 6)],
         quiz=[
    ...
    dict(slug="s2e03", no=8, title="백만 마리 개구리?", theme="이집트 — 큰 수의 이름", lv=2,
         novel="08_백만마리개구리_큰수.txt",
         pages=["S2E03_백만마리개구리_9컷_%d쪽.png" % i for i in range(1, 6)],
         quiz=[
    ...
    dict(slug="s2e04", no=9, title="예순까지 세는 손", theme="바빌론 — 60진법과 자릿값", lv=3,
         novel="09_예순까지세는손_60진법.txt",
         pages=["S2E04_예순까지세는손_9컷_%d쪽.png" % i for i in range(1, 6)],
         quiz=[
    ...
    dict(slug="s2e05", no=10, title="빵 셋, 사람 넷", theme="이집트 — 분수의 탄생", lv=3,
         novel="10_빵셋사람넷_분수.txt",
         pages=["S2E05_빵셋사람넷_9컷_%d쪽.png" % i for i in range(1, 6)],
         quiz=[
...
KHIST_EPISODES = [
    ...
    dict(slug="gojoseon_e02", title="2화. 1,908살 임금님의 은퇴 여행", lv=1,
         subtitle="단군왕검 하편 — 산신이 되다",
         pages=["고조선2화_1908살임금님의은퇴여행_9컷_%d쪽.png" % i for i in range(1, 6)],
         intro="나라를 연 임금에게는 걱정이 세 가지 있었습니다. 해마다 넘치는 물, 모자라는 곡식, "
               "그리고 말은 있는데 적어 둘 글자가 없다는 것이었습니다. 임금은 신하들에게 그 일을 맡기고, "
               "아들들에게는 성을 쌓고 바다 건너 사신으로 다녀오는 일을 맡깁니다. "
               "그리고 아주 긴 세월이 흐른 뒤, 임금은 아사달로 돌아가 산신이 되었다고 전합니다.",
         notes=[("전승", "단군왕검이 1,500년 동안 나라를 다스리다 도읍을 장당경으로 옮겼고, 뒤에 아사달로 돌아가 산신이 되었으며 1,908세를 누렸다는 이야기는 《삼국유사》(1281년경)에 적힌 그대로입니다."),
                ("전승", "마니산 참성단에서 하늘에 제사를 지냈다는 것과 세 아들이 삼랑성을 쌓았다는 이야기는 《고려사》에 실려 있습니다. 참성단은 아래가 둥글고 위가 네모납니다."),
                ("전승", "맏아들 부루를 중국 우임금이 연 도산 모임에 사신으로 보냈다는 이야기는 《세종실록지리지》와 《응제시주》에 실려 있습니다."),
                ("야사", "팽우가 물길을 다스렸다는 이야기는 《규원사화》에 나옵니다. 이 책은 진짜 옛 책인지 후대에 지어낸 책인지 학자들 사이에 다툼이 있습니다. 역사 시험용이 아니라 옛이야기로 즐기세요."),
                ("전승", "'주나라가 기자를 조선에 봉했다'는 말은 중국 책들에 적힌 주장입니다. 오늘날 학자들은 이를 사실로 보지 않으며, 1964년부터 교과서에서도 빠졌습니다."),
                ("기록", "해마다 개천절에 강화 참성단에서 전국체육대회 성화를 채화하는 것은 지금도 실제로 하는 일입니다."),
                ("상상", "학자들은 단군을 한 사람의 이름이 아니라 제사장과 임금을 함께 뜻하는 칭호로 봅니다. 1,908년은 여러 대의 임금을 한 이름으로 묶어 전한 것으로 풀이하며, 기원전 2333년이라는 연도도 후대에 거꾸로 계산해 붙인 것입니다.")],
         quiz=[{"q": "《삼국유사》에서 단군왕검이 마지막에 무엇이 되었다고 적혀 있나요?",
                "a": ["산신", "하늘의 별", "큰 나무", "바다의 용"], "c": 0},
               {"q": "참성단의 생김새로 맞는 것은?",
                "a": ["아래는 둥글고 위는 네모나다", "위아래 모두 둥글다",
                      "위아래 모두 네모나다", "세모난 탑 모양이다"], "c": 0},
               {"q": "세 아들이 쌓았다고 전하는 성의 이름은?",
                "a": ["삼랑성", "평양성", "왕검성", "장당경"], "c": 0}]),
